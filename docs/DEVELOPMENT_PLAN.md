...
Status re-reviewed 2026-09-17 at HEAD `8923ea2102bf88fe33afbf0f76e6c1c9957e154b`; the only change after the 2026-09-16 reviewed source is documentation, and `src/` remains `b0a45a0c3c4c7f38ac72f6dab134d2403da6f14f`. A fresh production build passes with three warnings, while seven emitted PNGs remain absent from the serving manifest. A network-denying execution of the production normalization closures reconfirmed nested-invoice zeroing and payload-owner fallback. Authority: [architecture](ARCHITECTURE.md), [executed review](DEVELOPMENT_REVIEW_2026-09-17.md). This plan is work to implement, not completed functionality. **Next repair:** Batch 1A before any real invoice or payment testing.

## Batch 1A — Invoice/ride adapters and reproducible gate (P1, release blocker)

- Add pure versioned normalized contracts in `src/api/contracts.js`; retain wallet transport in `src/api/nexusAPI.js`. Decode the current invoice `json` payload, preserve canonical register metadata and token identity, and require valid exact amount/items/recipient/account/status. For rides, require canonical `address` and `owner`; reject instead of falling back from missing `owner` to passenger-controlled `passenger-genesis`, and reject a redundant claim that differs from canonical ownership. Support another shape only through an explicit tested compatibility branch, never zero/empty success defaults.
- Add `test/contracts.test.js`, `test/api-envelopes.test.js`, `test/network-deny.js`, `test/package-inventory.test.js`, `test`/`lint` scripts in `package.json`, and `.github/workflows/ci.yml`. Keep NexusInterface-compatible versions; do not blindly upgrade the dependency tree.
...
## Batch 1B — Verified invoice issuance and settlement (P1, release blocker)

- Add `src/services/rideSettlement.js`, `src/store/rideIntents.js` and `test/ride-settlement.test.js`; move `Driver.handleCreateInvoice` orchestration out of `src/App/Driver.js`. Persist a ride-scoped issuance intent, reject ineligible/cancelled/paid requests after an authoritative reread, and retain the returned invoice address/txid before any taxi-state projection.
...
- Move `Passenger.handlePayInvoice` orchestration out of `src/App/Passenger.js`. Persist public intent scoped to profile/network/ride/invoice, freeze accepted terms, validate issuer/account/token/recipient/amount and reread authoritative state immediately before payment.
...
## Batch 1C — Packaging (P1)

- Update `webpack.config.babel.js` and `nxs_package.json`; generate an explicit manifest from emitted runtime assets or inline map images deliberately. Never add unsupported wildcard entries.
...
## Batch 2 — Bounded discovery and schema/privacy migration (P2; before public deployment)
